...
router.get('/', validate(schema.listQuerySchema, 'query'), async (req, res, next) => {
  try { res.json(await service.list({ actor: req.actor, filters: {
    q: req.query.q, status: req.query.status, urgency: req.query.urgency, category: req.query.category, mine: req.query.mine === 'true',
...
router.post('/:id/offers', idempotent(), validate(schema.idParamSchema, 'params'), validate(schema.offerSchema), async (req, res, next) => {
  try { res.status(201).json(await service.offerHelp({ actor: req.actor, id: req.params.id, note: req.body.note })); } catch (err) { next(err); }
});
