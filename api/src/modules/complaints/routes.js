...
// Ai cũng báo cáo được — không requireRole ở đây. Vai chỉ cần cho XỬ LÝ (GET
// danh sách, PATCH quyết định), cùng đúng hai vai join-requests đã dùng cho
// hàng đợi duyệt (approver, content_ops — xem modules/join-requests/routes.js).
...
router.get('/', requireRole('approver', 'content_ops'), validate(schema.listQuerySchema, 'query'), async (req, res, next) => {
  try { res.json(await service.list({ actor: req.actor, status: req.query.status ?? null, page: req.query.page, limit: req.query.limit })); }
  catch (err) { next(err); }
});
...
router.patch('/:id', requireRole('approver', 'content_ops'), validate(schema.idParamSchema, 'params'), validate(schema.decideSchema), async (req, res, next) => {
  try { res.json(await service.decide({ actor: req.actor, id: req.params.id, status: req.body.status, note: req.body.note })); }
  catch (err) { next(err); }
});
