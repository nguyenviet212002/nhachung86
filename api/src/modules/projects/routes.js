...
router.get('/', validate(schema.listQuerySchema, 'query'), async (req, res, next) => {
  try {
    res.json(await service.list({
      actor: req.actor,
      filters: { q: req.query.q, status: req.query.status, mine: req.query.mine === 'true' },
      page: req.query.page,
...
router.post('/:id/join', validate(schema.idParamSchema, 'params'), async (req, res, next) => {
  try { res.json(await service.join({ actor: req.actor, id: req.params.id })); } catch (e) { next(e); }
});
